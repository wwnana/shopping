...
$(function() {
	...
	*/
	...
	//给减号增加click事件
	$(".jian").click(function(){
		var id = $(this).attr("id").substring(0,32);//获取cartItemId
		var quantity = $("#"+id+"Quantity").val();//获取页面的quantity
		if(quantity == 1){
			if(confirm("确定是否删除该条目？")){
...
function sendUpdateQuantity(id,quantity){
	$.ajax({
	    ...
	    type:"post",
	    dataType:"json",
	    success:function(result){//CartItemServlet 返回的sb
...
<c:forEach items="${cartItemList }" var="cartItem">
	<tr align="center">
		...
		<td align="left" width="70px">
			<a class="linkImage" href="<c:url value='/ProductServlet?method=load&pid=${cartItem.product.pid }'/>"><img border="0" width="54" align="top" src="<c:url value='/${cartItem.product.image_b }'/>"/></a>
		</td>
		<td align="left" width="400px">
		    <a href="<c:url value='/ProductServlet?method=load&pid=${cartItem.product.pid }'/>"><span>${cartItem.product.pname }</span></a>
		</td>
		<td><span>&yen;<span class="currPrice">${cartItem.product.price }</span></span></td>
		<td>
